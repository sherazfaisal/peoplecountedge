...
def show_label_on_frame_and_counter(detections, frame, labels, colors):
    ...
    if ENABLE_ANIMAL_COUNT and SEND_COUNTER_TO_IOT_HUB: TEXT_TO_SEND = {"id":DEVICE_ID,"d":date_,"t":time_,"pc":str(animal_count),"ip":str(CAMERA_SOURCE)}
    if ENABLE_VEHICLE_COUNT and SEND_COUNTER_TO_IOT_HUB: TEXT_TO_SEND = {"id":DEVICE_ID,"d":date_,"t":time_,"pc":str(vehicle_count),"ip":str(CAMERA_SOURCE)}
