...
class KeyClipWriter:
    ...
    def start_recording_event(self):
        ...
        if not self.recording:
            ...
            p = "{}/{}.mkv".format(OUTPUT_PATH, timestamp.strftime("%Y%m%d-%H%M%S"))
            self.start(p, cv2.VideoWriter_fourcc(*CODEC), SET_FPS)

    def update_recording_event(self, frame,updateConsecFrames=True):
        global CONSEC_FRAMES
        if updateConsecFrames:
            CONSEC_FRAMES += 1
        # update the key frame clip buffer
        self.update(frame)
        if self.recording and CONSEC_FRAMES == self.bufSize:
            self.finish()
